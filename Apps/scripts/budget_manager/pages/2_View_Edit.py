...
def run():
    ...
    # -------------------------------
    # Ensure database folder & table exist
    # -------------------------------
    ensure_db_exists()
    ...
    with col1:
        month_filter = st.selectbox("Filter by month (optional)", ["-- all --"] + months, index=0)

    with col2:
        category_filter = st.selectbox("Filter by category (optional)", ["-- all --"] + categories, index=0)

    with col3:
        subcategory_filter = st.selectbox("Filter by subcategory (optional)", ["-- all --"] + subcategories, index=0)

    with col4:
        entry_type_filter = st.selectbox("Entry type", ["-- all --", "budget", "income"], index=0)
    ...
    df = fetch_entries(month=month_val, category=cat_val, subcategory=sub_val, entry_type=et_val)
    ...
    editable_df = st.data_editor(
        ...
        disabled=["id"],
        column_config={
            "entry_type": st.column_config.SelectboxColumn("Entry Type", options=["budget", "income"]),
    ...
    # Save changes button
    # -------------------------------
    if not changed_rows.empty and st.button("Save changes"):
